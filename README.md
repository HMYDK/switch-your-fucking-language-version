...
SYFLV 通过 **生成环境脚本** 的方式来管理语言版本，而非直接修改你的 Shell 配置。
...
│     ↓                                                           │
│  4. 生成脚本                                                     │
│     写入 ~/.config/runtimepilot/{language}_env.sh               │
│     内容: export JAVA_HOME="/path/to/jdk"                       │
│     ↓                                                           │
│  5. Shell 生效                                                   │
│     新终端会话自动 source 脚本，获得更新的环境变量                 │
└─────────────────────────────────────────────────────────────────┘
```
...
| 设计选择 | 好处 |
|---------|-----|
| 生成独立脚本而非修改 `.zshrc` | 不污染你的配置文件，卸载干净 |
| 每种语言独立的 `*_env.sh` | 可选择性启用，互不干扰 |
| 仅设置环境变量 | 无需安装 shim，无运行时性能损耗 |
